...
<head>
    ...
    <meta name="description" content="Styrk bedriften med Google Workspace. Akarit tilbyr ekspertise innen oppsett, migrering og support for økt samarbeid og produktivitet.">
    ...
    <style>
        /* --- CSS Styling (Akari Inspired - Mobil/Anker Fiks) --- */
        :root {
            ...
            --border-radius: 15px;
            --header-height: 70px; /* Estimat for header høyde - juster om nødvendig */
        }
        ...
        * { box-sizing: border-box; }
        ...
        /* --- Header --- */
        header { background-color: var(--dark-color); padding: 15px 0; position: sticky; top: 0; z-index: 1000; box-shadow: 0 2px 5px rgba(0,0,0,0.1); height: var(--header-height); /* Definer høyde */ display: flex; align-items: center; }
        ...
        .logo .sub-logo { font-size: 0.45em; opacity: 0.8; font-weight: 400; margin-left: 7px; white-space: nowrap; color: #01938b; }
        ...
        .hero::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(6, 79, 85, 0.50); z-index: 0; }
        .hero .container { position: relative; z-index: 1; }
        .hero h1 { font-family: var(--heading-font); font-size: 3.2em; font-weight: 700; color: var(--text-color-light); margin-bottom: 25px; text-shadow: 1px 1px 5px rgba(0,0,0,0.5); }
        .hero h1 span { color: var(--primary-color); }
        .hero p { font-size: 1.25em; color: var(--text-color-light); max-width: 780px; margin: 0 auto 40px auto; line-height: 1.8; }

        /* --- Knapper --- */
        .cta-button { display: inline-block; background-color: var(--primary-color); color: #fff; padding: 15px 35px; text-decoration: none; border-radius: var(--border-radius); font-weight: 600; transition: background-color 0.3s ease, transform 0.2s ease, box-shadow 0.3s ease; font-size: 1.1em; border: none; box-shadow: 0 4px 10px rgba(0, 169, 157, 0.3); cursor: pointer; }
        ...
        /* Spesifikk margin for den andre knappen i hero (for desktop) */
        ...
        section {
            padding: 80px 0;
            width: 100%;
            scroll-margin-top: calc(var(--header-height) + 20px); /* VIKTIG: Forteller nettleseren å legge til luft over seksjonen ved anker-scroll */
        }
        ...
        /* --- Kort (Funksjoner & Produkter - uendret) --- */
        ...
        @media (max-width: 768px) {
            ...
            header .container { flex-direction: column; text-align: center; height: auto; /* La høyden justeres */ padding-bottom: 10px;}
            .logo { margin-bottom: 15px; }
            nav ul { margin-top: 10px; justify-content: center; flex-wrap: wrap; }
            nav ul li { margin: 5px 10px; }
            .hero { padding: 120px 0 100px 0; } .hero h1 { font-size: 2.4em; } .hero p { font-size: 1.1em; max-width: 90%; }
            section h2 { font-size: 2em; }
            .features-grid, .products-grid { grid-template-columns: 1fr; gap: 25px; }
            .contact-form { padding: 40px 30px; }
            :root { --header-height: auto; /* Fjern fast høyde på mobil */ }
             section { scroll-margin-top: 80px; /* Juster for mobil om nødvendig */ }
        }

        @media (max-width: 576px) {
            .hero { padding: 100px 20px 80px 20px; }
            .hero h1 { font-size: 2em; } .hero p { font-size: 1em; }
            /* Styling for hero-knapper på mobil */
            .hero .cta-button {
                display: block; /* Stable vertikalt */
                width: fit-content; /* Tilpass bredde til innhold */
                max-width: 80%; /* Unngå at de blir for brede */
                margin: 15px auto; /* Sentrer og legg til vertikal avstand */
            }
            /* Fjern spesifikk margin for andre knapp på mobil */
            .hero .cta-button + .cta-button {
                 margin-left: auto; /* Overstyr desktop margin */
            }

            section h2 { font-size: 1.8em; }
            .cta-button { padding: 14px 30px; font-size: 1em; } /* Generell knappjustering for mobil */
            nav ul li a { font-size: 0.9em; } .logo { font-size: 1.7em; }
            .contact-form { padding: 30px 20px; margin-left: 15px; margin-right: 15px; }
            .feature-item, .product-card { padding: 30px; margin-left: 15px; margin-right: 15px;}
        }
...
<body>
    ...
    <section class="hero" id="hjem">
        ...
        <div class="container">
             ...
             <a href="#produkter" class="cta-button secondary">Se Verktøyene</a> <!-- Fjernet inline style -->
        </div>
    </section>

    <!-- Resten av HTML-seksjonene (fordeler, produkter, hvorfor-oss, kontakt, footer) er uendret -->
    <section id="fordeler">
          <div class="container">
    ...
    <footer>
        <div class="container">
            <p>&copy; <?php echo date("Y"); ?> Akarit (av Akari AS). Alle Rettigheter Reservert. | Sertifisert Google Workspace Partner</p>
        </div>
    </footer>

    <script>
    // --- JavaScript for Smooth Scroll (Justert Offset) ---
    document.querySelectorAll('header nav a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (targetElement) {
                // Bruk getBoundingClientRect for mer nøyaktig posisjon relativt til viewport
                const elementPosition = targetElement.getBoundingClientRect().top;
                // Hent header høyde dynamisk (viktig hvis den endres på mobil etc.)
                const headerOffset = document.querySelector('header').offsetHeight;
                // Kalkuler offset basert på nåværende scroll + element posisjon - header - litt ekstra padding
                const offsetPosition = window.pageYOffset + elementPosition - headerOffset - 25; // Justert offset (prøv 25px)

                window.scrollTo({
                    top: offsetPosition,
                    // Behavior: smooth håndteres nå av CSS `scroll-behavior: smooth;` på <html>
                    // behavior: "smooth" // Kan beholdes, men CSS er ofte bedre
                });

                // Aktiv link håndtering (uendret)
    ...
    // Aktiv link on scroll (litt forenklet, da scroll-margin-top hjelper)
    const sections = document.querySelectorAll('section[id]');
    const navLinks = document.querySelectorAll('header nav a');
    const headerHeightEstimate = 80; // Bruk et estimat for scroll-check

    window.addEventListener('scroll', () => {
        let currentSectionId = '';
        sections.forEach(section => {
            const sectionTop = section.offsetTop - headerHeightEstimate; // Bruk estimat her
        ...
        if (window.pageYOffset < sections[0].offsetTop - headerHeightEstimate * 2) {
             currentSectionId = ''; // Ingen aktiv når helt på topp
        }
        ...
        navLinks.forEach(link => {
         ...
         // Hvis ingen er aktiv, og vi er på toppen, aktiver første link (hvis den finnes)
        // if (!foundActive && window.pageYOffset < 100 && navLinks.length > 0) {
        //      const homeLink = document.querySelector('header nav a[href="#hjem"]'); // Anta at hero har id="hjem"
        //      if(homeLink) homeLink.classList.add('active');
        //      else navLinks[0].classList.add('active'); // Fallback til første
        // }
    });
    console.log("Akarit Google Workspace side lastet (v9 - Mobil/Anker fiks).");
    </script>
